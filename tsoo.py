...
rows_to_delete = []
words_for_delete = ['субботник', 'под загрузку', 'под погрузку', 'установка']
for row in new_sh.iter_rows(min_row=2):
    if '789' in (str(row[2].value.lower()))[:3]:
        rows_to_delete.append(row[0].row)
    if 'сигнальный' in row[3].value.lower() or 'вкп' in row[3].value.lower():
        rows_to_delete.append(row[0].row)
    if 'помешочный сбор' in row[6].value.lower():
        rows_to_delete.append(row[0].row)
    if any(word in row[14].value.lower() for word in words_for_delete):
        rows_to_delete.append(row[0].row)
